...
{
	char sqlbuf[256] = {0};
	sprintf_s( sqlbuf, sizeof(sqlbuf), "DELETE FROM dev_relation where device_type=%d and device_id=%d", device_type, device_id );
	db->exec( sqlbuf );
...
bool GSIOTConfig::ClearAllRelation_SpecType( IOTDeviceType device_type )
{
	...
	char sqlbuf[256] = {0};
	sprintf_s( sqlbuf, sizeof(sqlbuf), "DELETE FROM dev_relation where device_type=%d", device_type );
	db->exec( sqlbuf );
...
bool GSIOTConfig::SaveRelation( IOTDeviceType device_type, int device_id, const deflstRelationChild &ChildList )
{
	...
	// 先所有置0
	char sqlbuf[1024] = {0};
	sprintf_s( sqlbuf, sizeof(sqlbuf), "update dev_relation set enable=0 where device_type=%d and device_id=%d", device_type, device_id );
	db->exec( sqlbuf );
	...
	for( deflstRelationChild::const_iterator it=ChildList.begin(); it!=ChildList.end(); ++it, ++sort_no )
	{
		...
		{
			sprintf_s( sqlbuf, sizeof(sqlbuf), "update dev_relation set enable=1, sort_no=%d, flag=%d where device_type=%d and device_id=%d and child_dev_type=%d and child_dev_id=%d and child_dev_subid=%d", 
...
{
	char sqlbuf[1024] = {0};
	sprintf_s( sqlbuf, sizeof(sqlbuf), "SELECT id FROM dev_relation where device_type=%d and device_id=%d and child_dev_type=%d and child_dev_id=%d and child_dev_subid=%d", 
...
bool GSIOTConfig::db_Update_vobj_config( const stru_vobj_config &cfg )
{
	...
	sprintf_s( sqlbuf, sizeof(sqlbuf), "update vobj_config set enable=%d, vobj_type=%d, vobj_flag=%d, name='%s', ver='%s',"\
...
bool GSIOTConfig::db_Delete_vobj_config( const IOTDeviceType vobj_type, const int id )
{
	...
	sprintf_s( sqlbuf, sizeof(sqlbuf), "DELETE FROM vobj_config WHERE vobj_type=%d and id=%d", vobj_type, id );
	db->exec( sqlbuf );
